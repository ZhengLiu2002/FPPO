...
# 1. 尝试导入配置类 (RslRlOnPolicyRunnerCfg) 和环境包装器 (RslRlVecEnvWrapper)
try:
    ...
    try:
        ...
        raise ImportError(
            "CRITICAL: 无法导入 RslRlOnPolicyRunnerCfg 或 RslRlVecEnvWrapper。请检查 rl_algorithms 路径。\n"
            f"详细错误: {e}"
        )

print(f"[INFO] Successfully imported: Runner={OnPolicyRunner.__name__}, Wrapper={RslRlVecEnvWrapper.__name__}")
# =================================================================================
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # create runner from local rsl-rl fork
