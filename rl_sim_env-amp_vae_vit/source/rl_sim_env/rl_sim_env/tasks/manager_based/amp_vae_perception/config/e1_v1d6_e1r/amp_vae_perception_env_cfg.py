...
from rl_sim_env.tasks.manager_based.amp_vae_perception.amp_vae_perception_base_env_cfg import (
    AmpVaePerceptionEnvCfg,
)
from rl_sim_env.tasks.manager_based.amp_vae_perception.config.e1_v1d6_e1r.config_summary import (
    ROBOT_BASE_LINK,
    ROBOT_CFG,
    ROBOT_FOOT_NAMES,
    ConfigSummary,
...
from rl_sim_env.tasks.manager_based.common.terrain.config import (
    AMP_VAE_PERCEPTION_TERRAIN_CFG,
)


@configclass
class E1V1d6E1rAmpVaePerceptionEnvCfg(AmpVaePerceptionEnvCfg):
    def __post_init__(self):
        ...
        self.scene.terrain = AMP_VAE_PERCEPTION_TERRAIN_CFG
        ...
        # # command settings
        # self.commands.base_velocity = create_uniform_velocity_command_plus_cfg(
        #     rel_standing_envs=self.config_summary.command.rel_standing_envs,
        #     rel_heading_envs=self.config_summary.command.rel_heading_envs,
        #     rel_yaw_envs=self.config_summary.command.rel_yaw_envs,
        #     heading_command=self.config_summary.command.heading_command,
        ...
        #     terrain_part=int(self.config_summary.env.num_terrains_percent * self.config_summary.env.num_envs),
        # )
        ...
        # vae obs
        self.observations.vae_obs.base_ang_vel.scale = self.config_summary.observation.scale.base_ang_vel
        self.observations.vae_obs.projected_gravity.scale = self.config_summary.observation.scale.projected_gravity
        self.observations.vae_obs.velocity_commands.params["scale"] = self.config_summary.observation.scale.vel_command
        self.observations.vae_obs.joint_pos.scale = self.config_summary.observation.scale.joint_pos
        self.observations.vae_obs.joint_vel.scale = self.config_summary.observation.scale.joint_vel
        ...
        # vae obs
        self.observations.vae_obs.base_ang_vel.noise = Unoise(
            n_min=-base_ang_vel_noise,
            n_max=base_ang_vel_noise,
        )
        self.observations.vae_obs.projected_gravity.noise = Unoise(n_min=-gravity_noise, n_max=gravity_noise)
        self.observations.vae_obs.joint_pos.noise = Unoise(
            n_min=-joint_pos_noise,
            n_max=joint_pos_noise,
        )
        self.observations.vae_obs.joint_vel.noise = Unoise(
            n_min=-joint_vel_noise,
            n_max=joint_vel_noise,
        )
...
@configclass
class E1V1d6E1rAmpVaePerceptionEnvCfg_PLAY(E1V1d6E1rAmpVaePerceptionEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        ...
        self.observations.vae_obs.enable_corruption = False
...
class E1V1d6E1rAmpVaePerceptionEnvCfg_REPLAY_AMPDATA(E1V1d6E1rAmpVaePerceptionEnvCfg_PLAY):
    def __post_init__(self):
        super().__post_init__()
        self.scene.terrain.terrain_type = "plane"
        self.scene.terrain.terrain_generator = None
