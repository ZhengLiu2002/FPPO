...
from rl_sim_env.tasks.manager_based.amp_vae_vit.config.e1_v1d6_e1r.config_summary import (
    ROBOT_BASE_LINK,
    ROBOT_CFG,
    ROBOT_FOOT_NAMES,
    ConfigSummary,
...
from rl_sim_env.tasks.manager_based.common.terrain.config import AMP_VAE_VIT_TERRAIN_CFG

@configclass
class E1V1d6E1rAmpVaeVitEnvCfg(AmpVaeVitEnvCfg):
    def __post_init__(self):
        ...
        self.scene.terrain = AMP_VAE_VIT_TERRAIN_CFG
        ...
        for key, item in self.scene.terrain.terrain_generator.sub_terrains.items():
            command_ids[key] = list(range(env_start, env_start + int(item.proportion * num_envs)))
            env_start += int(item.proportion * num_envs)
...
@configclass
class E1V1d6E1rAmpVaeVitEnvCfg_PLAY(E1V1d6E1rAmpVaeVitEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
...
class E1V1d6E1rAmpVaeVitEnvCfg_REPLAY_AMPDATA(E1V1d6E1rAmpVaeVitEnvCfg_PLAY):
    def __post_init__(self):
        super().__post_init__()
