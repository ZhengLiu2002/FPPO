...
class AmpVaePerceptionRLEnv(ManagerBasedEnv, gym.Env):
    ...
    cfg: AmpVaePerceptionEnvCfg
    """Configuration for the environment."""

    def __init__(self, cfg: AmpVaePerceptionEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.obs_actor_delay_buffer = DelayBuffer(self.obs_max_delay, self.num_envs, self.device)
        self.obs_vae_delay_buffer = DelayBuffer(self.obs_max_delay, self.num_envs, self.device)
        # self.obs_amp_delay_buffer = DelayBuffer(self.obs_max_delay, self.num_envs, self.device)
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        if len(self.recorder_manager.active_terms) > 0:
            ...
            if self.cfg.config_summary.env.clip_obs is not None:
                for term in self.obs_buf.values():
                    term[:] = torch.clip(
        ...
        terminal_amp_states = self.obs_buf["amp_obs"][reset_env_ids]
        self.obs_buf = self.observation_manager.compute()
        self.obs_buf["critic_obs"] = self.obs_critic_delay_buffer.compute(self.obs_buf["critic_obs"])
        self.obs_buf["actor_obs"] = self.obs_actor_delay_buffer.compute(self.obs_buf["actor_obs"])
        self.obs_buf["vae_obs"] = self.obs_vae_delay_buffer.compute(self.obs_buf["vae_obs"])
        # self.obs_buf["amp_obs"] = self.obs_amp_delay_buffer.compute(self.obs_buf["amp_obs"])
        if self.cfg.config_summary.env.clip_obs is not None:
            for term in self.obs_buf.values():
                term[:] = torch.clip(
                    term, min=-self.cfg.config_summary.env.clip_obs, max=self.cfg.config_summary.env.clip_obs
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        # reset the obs delay buffers
        time_lags = torch.randint(
            low=self.obs_min_delay,
            high=self.obs_max_delay + 1,
            size=(len(env_ids),),
            dtype=torch.int,
            device=self.device,
        )
        self.obs_critic_delay_buffer.set_time_lag(time_lags, env_ids)
        self.obs_actor_delay_buffer.set_time_lag(time_lags, env_ids)
        self.obs_vae_delay_buffer.set_time_lag(time_lags, env_ids)
        # self.obs_amp_delay_buffer.set_time_lag(time_lags, env_ids)
        self.obs_critic_delay_buffer.reset(env_ids)
        self.obs_actor_delay_buffer.reset(env_ids)
        self.obs_vae_delay_buffer.reset(env_ids)
        # self.obs_amp_delay_buffer.reset(env_ids)
