...
from rl_sim_env.tasks.manager_based.amp_vae_vit.amp_vae_vit_base_env_cfg import AmpVaeVitEnvCfg


class AmpVaeVitRLEnv(ManagerBasedEnv, gym.Env):
    ...
    def __init__(self, cfg: AmpVaeVitEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # -- init buffers
        self.obs_dict = dict()
        self.obs_dict["critic_obs"] = torch.zeros(self.num_envs, cfg.config_summary.env.num_critic_obs, device=self.device, dtype=torch.float32)
        ...
        self.vae_obs_history = DataBuffer(
            self.num_envs, cfg.config_summary.env.num_vae_obs, cfg.config_summary.env.obs_history_length, self.device
        )
        ...
        self.obs_actor_projected_gravity_delay_buffer = DataBuffer(
            self.num_envs, 3, self.obs_max_delay + 1, self.device
        )
        ...
        self.obs_actor_joint_pos_delay = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self.obs_actor_joint_vel_delay = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        self.actions_history.insert(action)
        # process actions
        ...
        for _ in range(self.cfg.decimation):
            ...
            obs_actor_delay = self.observation_manager.compute_group("actor_obs")
            self.obs_actor_base_ang_vel_delay_buffer.insert(obs_actor_delay["base_ang_vel"])
            self.obs_actor_projected_gravity_delay_buffer.insert(obs_actor_delay["projected_gravity"])
            self.obs_actor_joint_pos_delay_buffer.insert(obs_actor_delay["joint_pos"])
            self.obs_actor_joint_vel_delay_buffer.insert(obs_actor_delay["joint_vel"])
        ...
        if len(self.recorder_manager.active_terms) > 0:
            ...
            # self.obs_dict = self.observation_manager.compute()
            if self.cfg.config_summary.env.clip_obs is not None:
                for term in self.obs_dict.values():
                    term[:] = torch.clip(
                        term, min=-self.cfg.config_summary.env.clip_obs, max=self.cfg.config_summary.env.clip_obs
                    )
        ...
        if len(reset_env_ids) > 0:
            self.episode_reward_term_buf = self.episode_reward_buf.clone()
        ...
        self.obs_dict["critic_obs"] = self.observation_manager.compute_group("critic_obs")
        self.obs_dict["amp_obs"] = self.observation_manager.compute_group("amp_obs")
        obs_actor_current = self.observation_manager.compute_group("actor_obs")
        obs_actor_base_ang_vel = self.obs_actor_base_ang_vel_delay_buffer.get_data_his(his_list=self.obs_actor_base_ang_vel_delay)
    ...
    def _reset_idx(self, env_ids: Sequence[int]):
        ...
        self.vae_obs_history.reset(env_ids)
        # reset the obs delay
        self.obs_actor_base_ang_vel_delay[env_ids] = torch.randint(low=self.obs_min_delay, high=self.obs_max_delay + 1, size=(len(env_ids),), dtype=torch.long, device=self.device)
        self.obs_actor_projected_gravity_delay[env_ids] = torch.randint(low=self.obs_min_delay, high=self.obs_max_delay + 1, size=(len(env_ids),), dtype=torch.long, device=self.device)
        self.obs_actor_joint_pos_delay[env_ids] = torch.randint(low=self.obs_min_delay, high=self.obs_max_delay + 1, size=(len(env_ids),), dtype=torch.long, device=self.device)
        self.obs_actor_joint_vel_delay[env_ids] = torch.randint(low=self.obs_min_delay, high=self.obs_max_delay + 1, size=(len(env_ids),), dtype=torch.long, device=self.device)

        self.obs_actor_base_ang_vel_delay_buffer.reset(env_ids)
        self.obs_actor_projected_gravity_delay_buffer.reset(env_ids)
        self.obs_actor_joint_pos_delay_buffer.reset(env_ids)
        self.obs_actor_joint_vel_delay_buffer.reset(env_ids)
