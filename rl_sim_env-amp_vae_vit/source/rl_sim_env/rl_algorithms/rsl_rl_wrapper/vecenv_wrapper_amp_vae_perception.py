...
from rl_sim_env.envs import AmpVaePerceptionRLEnv


class AmpVaePerceptionVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: AmpVaePerceptionRLEnv, clip_actions: float | None = None, clip_obs: float | None = None):
        ...
        if not isinstance(env.unwrapped, AmpVaePerceptionRLEnv):
            raise ValueError(
                f"The environment must be inherited from AmpVaePerceptionRLEnv. Environment type: {type(env)}"
            )
    ...
    @property
    def unwrapped(self) -> AmpVaePerceptionRLEnv:
