...
from rl_sim_env.envs import AmpVaeRLEnv


class AmpVaeVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: AmpVaeRLEnv, clip_actions: float | None = None, clip_obs: float | None = None):
        ...
        if not isinstance(env.unwrapped, AmpVaeRLEnv):
            raise ValueError(f"The environment must be inherited from AmpVaeRLEnv. Environment type: {type(env)}")
    ...
    @property
    def unwrapped(self) -> AmpVaeRLEnv:
