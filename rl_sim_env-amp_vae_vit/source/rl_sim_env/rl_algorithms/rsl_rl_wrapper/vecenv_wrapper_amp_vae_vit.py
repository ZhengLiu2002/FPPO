...
from rl_sim_env.envs import AmpVaeVitRLEnv


class AmpVaeVitVecEnvWrapper(VecEnv):
    ...
    def __init__(self, env: AmpVaeVitRLEnv, clip_actions: float | None = None, clip_obs: float | None = None):
        ...
        if not isinstance(env.unwrapped, AmpVaeVitRLEnv):
            raise ValueError(f"The environment must be inherited from AmpVaeVitRLEnv. Environment type: {type(env)}")
        ...
        self.num_actions = self.cfg.config_summary.env.num_actions
    ...
    @property
    def unwrapped(self) -> AmpVaeVitRLEnv:
    ...
    def get_observations(self) -> tuple[torch.Tensor, dict]:
        return self.unwrapped.obs_dict
